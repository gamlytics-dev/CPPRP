...
INC_FILES := $(shell find $(INC_DIRS) -name *.h) 
...
all: preprocess $(BUILD_DIR)/${TARGET_BIN} $(BUILD_DIR)/libcpprp.so

$(BUILD_DIR)/${TARGET_BIN}: $(OBJS) 
	$(CXX) -o $@ $(CXXFLAGS) $(OBJS) $(LDFLAGS) -pthread

$(BUILD_DIR)/libcpprp.so: $(OBJS)
	$(CXX) -shared -o $@ $(OBJS) $(LDFLAGS) -pthread
